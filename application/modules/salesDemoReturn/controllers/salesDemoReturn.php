...

{
	 ...
	 $data= $this->db->query("select * from tbl_return_dtl where returnhdr='$id'");
...
public function send_remainder_mail_1()
...
public function send_remainder_mail()
{
	...
	foreach($remQuery->result() as $getResult)
	{
		...
		$sendMailQ=$this->db->query("select * from tbl_return_hdr where responsibility='$getResult->contact_id' AND return_due_date >= '$todays' ");
		$cntContact=$sendMailQ->row();

		//================qty validation====================
		$returnDtlQ=$this->db->query("select SUM(quantity) as dqty from tbl_return_dtl where returnhdr='$cntContact->returnid'");
		$getDtlSumQty=$returnDtlQ->row();

		$returnSerQ=$this->db->query("select SUM(quantity) as sqty from tbl_product_serial_log where inbound_id='$cntContact->returnid' and type='Demo Return'");
		$getSerSumQty=$returnSerQ->row();

		//===========================end=================

		//$todays=date('Y-m-d');
		//$date1=date_create($todays);
		//$date2=$cntContact->return_due_date;

		
		if($cntContact->responsibility==$getResult->contact_id && $getDtlSumQty->dqty!=$getSerSumQty->sqty)
		{
			...
			$this->email->bcc('kumar14mahto@gmail.com');
...
public function send_remainder_mail_test()
{
	...
	foreach($remQuery->result() as $getResult)
	{
		...
		//if($cntContact->responsibility==$getResult->contact_id)
		{
			...
			$this->email->to('kumar14mahto@gmail.com');
