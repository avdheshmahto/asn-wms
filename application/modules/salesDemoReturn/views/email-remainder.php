...
if($id!="")
{
  ...
  $fetchcomp=$this->db->query("select * from tbl_contact_m where contact_id='$id'");
...
<p>
<strong>Dear</strong> &nbsp;  <?=$fetchcompany->first_name;?><br />  
Please find the below detail of demo units issued under your responsibility.</p>
<br />If you have any query or found any discrepancy, please feel free to contact Jatin Chopra at jatin.chopra@sennheiser.com.</td>
</tr>
...
foreach($inviceQ->result() as $invoiceFetch)
{
  ...
  $getLog = $this->db->query("Select product_id,inbound_id,type,quantity from tbl_product_serial_log where type = 'Demo Return' and product_id='$invoiceFetch->product_id' and inbound_id='$invoiceFetch->returnhdr' ");
  $logdata = $getLog->row();
  $count=$getLog->num_rows(); 


if($logdata->quantity!='')
{
  $qnt=$invoiceFetch->quantity-$logdata->quantity;  
  //$qnt=$logdata->quantity;  
}
else
{
  $qnt=$invoiceFetch->quantity; 
}


  if(@$logdata->product_id == @$invoiceFetch->product_id && @$logdata->inbound_id == @$invoiceFetch->returnhdr && @$logdata->quantity==@$invoiceFetch->quantity) 
  {
