...
<tbody>
<?php
$query_hdr=$this->db->query("select * from tbl_return_hdr where returnid='".$_GET['id']."' or returnid='".$_GET['view']."' ");
$getHdr=$query_hdr->row();
?>

<input type="hidden" name="type" value="Demo Return" />
<?php


$z=1;
//$query_dtl=$this->db->query("select * from tbl_return_dtl where returnhdr='".$_GET['id']."' or returnhdr='".$_GET['view']."' ");
$query_dtl=$this->db->query("select *,SUM(quantity) as dtlQty from tbl_return_dtl where returnhdr='".$_GET['id']."' or returnhdr='".$_GET['view']."' group by product_id ");
...
$queryInbondQtyCnt=$this->db->query("select * from tbl_product_serial_log where product_id='$getProductName->Product_id' and inbound_id='".$_GET['id']."' and type='Demo Return'");
$getInbundQtyCnt=$queryInbondQtyCnt->num_rows();


	$queryInbondQty=$this->db->query("select *,SUM(quantity) as qtySum from tbl_product_serial_log where product_id='$getProductName->Product_id' and inbound_id='".$_GET['id']."' and type='Demo Return'");
...
{
	$LogSum=$invoiceFetch->dtlQty-$getInbundQty->qtySum;
}
else
{
	$LogSum=$invoiceFetch->dtlQty;
}
?>
...
<th><input type="number" step="any" name="new_quantity[]" <?php if($LogSum==0){?> readonly="readonly"<?php }?> onchange="checkQtyVal(this.id);" id="new_qty<?=$z;?>" class="form-control" style="width:150px;" /></th>
</tr>
<?php $z++;}?>
...
<th>&nbsp;</th>
<th>&nbsp;</th>
<th><input class="btn btn-primary btn-sm" name="save"  type="submit" value="Stock In" /></th>
...
//alert(zz);
	...
	getPalletQty(main_loc,pri,pri_id,asx);
