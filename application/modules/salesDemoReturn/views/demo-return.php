...
<tr>
		...
		<th><div style="width:130px;">RGP Gatepass No.</div></th>
...
{
	//echo $rows->returnid;
	// before was this query $returnDtlQ=$this->db->query("select *from tbl_return_dtl where returnhdr='$rows->returnid'");
	$returnDtlQ=$this->db->query("select SUM(quantity) as dqty from tbl_return_dtl where returnhdr='$rows->returnid'");
	$getDtl=$returnDtlQ->num_rows();
	$getDtlSumQty=$returnDtlQ->row();
	//echo $getDtlSumQty->dqty;
	
	// before was this query $returnSerQ=$this->db->query("select * from tbl_product_serial_log where inbound_id='$rows->returnid' and type='Demo Return'");
	$returnSerQ=$this->db->query("select SUM(quantity) as sqty from tbl_product_serial_log where inbound_id='$rows->returnid' and type='Demo Return'");
	$getSer=$returnSerQ->num_rows();
	$getSerSumQty=$returnSerQ->row();
	// before query was this if($getDtl==$getSer)
	//echo $getDtlSumQty->dqty."&nbsp;".$getSerSumQty->sqty;
?>
<tr class="gradeC record" <?php if($getDtlSumQty->dqty==$getSerSumQty->sqty)
	{?> style="display:none" <?php }?>>
<th><?php echo $rows->returnid;?></th>
<th><?php
$resQuery=$this->db->query("select *from tbl_contact_m where contact_id='$rows->responsibility'");
$getRes=$resQuery->row();
 echo $getRes->first_name;?></th>
