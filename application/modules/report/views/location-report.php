...
<h4 class="panel-title">ALL LOCATION REPORT </h4>
...
<?php
$location=$this->db->query("select *from tbl_location where STATUS='A'");
foreach($location->result() as $getData){
?>
<option value="<?=$getData->id;?>"><?=$getData->location_name;?></option>
<?php }?>
</select>
</div>
...
<a href="<?=base_url('report/Report/export_location_all').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&main_location='.$_GET['main_location'].'&location='.$_GET['location'].'&filter='.$_GET['filter'];?>" class="dt-button buttons-excel buttons-html5" style="margin: 0px 30px 0px 0px;"><strong>Excel</strong></a></div>
</div>

<div class="dataTables_length" id="DataTables_Table_0_length" style="margin: 0px 0px 0px 30px;">
<label> Show
<select name="DataTables_Table_0_length" aria-controls="DataTables_Table_0" id="entries"  url="<?=base_url('/report/Report/searchLocation').'?p_name='.$_GET['p_name'].'&sku_no='.$_GET['sku_no'].'&main_location='.$_GET['main_location'].'&location='.$_GET['location'].'&filter='.$_GET['filter'];?>" class="form-control input-sm">
...
?>
<tr class="gradeC record" <?php if($rows->product_id!=$fProQ12->Product_id){?> style="display:none" <?php }?>>
